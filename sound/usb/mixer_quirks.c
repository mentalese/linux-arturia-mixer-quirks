...
// Arturia Audiofuse
struct snd_audiofuse_mixer_entry {
    const char *name;
    unsigned int private_value;
    struct snd_kcontrol *kctl;  // 추가
};

static struct snd_audiofuse_mixer_entry snd_audiofuse_mixers[] = {
	{
		.name = "Input1 48V LED Switch",
		.private_value = (0x0001 << 8) | 0x00, // 0x0001(48V), 0x00(기본값)
	},
	{
		.name = "Main Speaker Mute LED Switch",
		.private_value = (0x0500 << 8) | 0x00, // 0x0500(mute), 0x00(기본값)
	},	
	
};

static int snd_audiofuse_control_init_all(struct usb_mixer_interface *mixer)
{
	struct usb_device *dev = mixer->chip->dev;
	...
	print_hex_dump(KERN_INFO, "Audiofuse Initial Data: ", DUMP_PREFIX_OFFSET, 16, 1, data, 178, false);

	int i;
	struct snd_kcontrol *kctl;
    for (i = 0; i < ARRAY_SIZE(snd_audiofuse_mixers); ++i) {
        kctl = snd_audiofuse_mixers[i].kctl; // 컨트롤 가져오기
        if (kctl) {
			unsigned int pval = kctl->private_value;	
			unsigned int wValue = (pval >> 8);
			int data_index;
			// wValue에 따라 다른 인덱스 사용
			if (wValue == 0x0001) {
				data_index = 0x3e;
			} else if (wValue == 0x0500) {
				data_index = 0x5c;
			} else {
				// 기본값 또는 다른 경우 처리
				data_index = 177; // 0x00
			...
			kctl->private_value = (wValue << 8) | data[data_index];
			dev_info(&dev->dev,
				"Audiofuse Initial private_value = %#x", kctl->private_value);
...
static int snd_audiofuse_control_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
    // 하위 1바이트를 저장(0 or 1)
...
static int snd_audiofuse_update_cur_val(struct usb_mixer_elem_list *list)
{
	struct snd_usb_audio *chip = list->mixer->chip;
	unsigned int pval = list->kctl->private_value;	
	unsigned int wValue = (pval >> 8);
	unsigned int data = pval & 0xFF; // 1바이트 데이터로 수정	
	int err;
	...
	dev_info(NULL,
		"Audiofuse Updated private_value = %#x", list->kctl->private_value);						
	print_hex_dump(KERN_INFO, "Audiofuse Updated Data: ", DUMP_PREFIX_OFFSET, 16, 1, receive, 178, false);							
...
static int snd_audiofuse_control_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct usb_mixer_elem_list *list = snd_kcontrol_chip(kcontrol);
    unsigned int oldval = kcontrol->private_value & 0xFF;
    unsigned int newval = ucontrol->value.integer.value[0]; // 0 또는 1만 가져야함
    unsigned int wValue = (kcontrol->private_value >> 8);
    int err;
    ...
    // 새로운 값을 저장
    kcontrol->private_value = (wValue << 8) | newval;
	
	err = snd_audiofuse_update_cur_val(list);
	return err < 0 ? err : 1;
}

// 전역 또는 구조체 내 추적용 변수
static bool snd_audiofuse_controls_registered = false;
static int snd_audiofuse_controls_create(struct usb_mixer_interface *mixer)
{
	if (snd_audiofuse_controls_registered) {
		return 0;
	}
	int i, err=0;
	...
	struct snd_kcontrol_new template = {
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.get = snd_audiofuse_control_get,
		.put = snd_audiofuse_control_put,		
		.info = snd_ctl_boolean_mono_info,
	};	
	...
	for (i = 0; i < ARRAY_SIZE(snd_audiofuse_mixers); ++i) {
		...
		template.name = snd_audiofuse_mixers[i].name;
		template.private_value = snd_audiofuse_mixers[i].private_value;

		err = add_single_ctl_with_resume(mixer, 0,
						 snd_audiofuse_update_cur_val, 
						 &template, &list);
		if (err < 0)
			return err;
		// snd_audiofuse_control_init_val(mixer, list->kctl);
		// snd_audiofuse_update_cur_val(list);
        // 컨트롤을 리스트에 저장
        snd_audiofuse_mixers[i].kctl = list->kctl;		
	}

    // 모든 컨트롤 등록 후 한 번만 초기화
    snd_audiofuse_control_init_all(mixer);	

	snd_audiofuse_controls_registered = true;
	return 0;
...
int snd_usb_mixer_apply_create_quirk(struct usb_mixer_interface *mixer)
{
	...
	case USB_ID(0x2b73, 0x000a): /* Pioneer DJ DJM-900NXS2 */
		...
		break;
	case USB_ID(0x1c75, 0xaf02): /* Arturia AudioFuse*/
		err = snd_audiofuse_controls_create(mixer);
		break;
	}	
